...
Network::Network(const LidarAddress& sensor)
  ...
  , interface_name_(sensor.interface_name())
  , socket_(io_service_)
{
...
error_t Network::open()
{
  ...
  if (!interface_name_.empty()) {
    const int rc = ::setsockopt(
      ...
      interface_name_.size() + 1);
    if (rc != 0) {
      std::cerr << "bind UDP socket to interface " << interface_name_
  ...
  if (!interface_name_.empty()) {
    std::cout << "socket interface is " << interface_name_ << std::endl;
  }
#endif
